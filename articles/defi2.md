...
The pools used for liquidity provision and mining have evolved into two categories: Pool 1 and Pool 2. The first, Pool 1, represents tokens that are not part of the protocol or governance structure and can be easily moved around to pools with the highest interest rate. These tokens generally have deeper liquidity, are more common, and would still exist without the farm. An example would be depositing `ETH-USDC` into SushiSwap. Neither of these tokens (`ETH` or `USDC`) have their own governance token, and they can easily move to another farm if the rates there are more attractive. This type of fickle capital behaviour can be called *mercenary capital* as it moves around to the highest bidder in a moments notice[^mercenary].
